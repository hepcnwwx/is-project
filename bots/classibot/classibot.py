...
class Bot:
	...
	def labelling(self, feature):
		probabilities = self.__model.predict_proba([feature])
		good_proba = probabilities[0][0]
		if good_proba >= 0.6:
			return "good"
		elif good_proba >= 0.4:
			return "ok"
	...
	def cut_down(self, state, moves):
		...
		for move in moves:
			if move[1] == None and move[0] != None:
				...
				# TODO: Add __model attribute
				label=self.labelling(feature)
				if label!="bad":
					new_list.append(move)
					if label=="good":
						good_list.append(move)
			else:
				new_list.append(move)
				good_list.append(move)
		# Delete all the moves that is not optimal according to the model
		if len(good_list)/len(moves)>0.3:
			#print("good chosed", "good",len(good_list),"new",len(new_list),"normal",len(moves))
			return good_list

		elif len(new_list) > 0:
			#print("new chosed", "good",len(good_list),"new",len(new_list),"normal",len(moves))
			return new_list

		else:
			#print("moves chosed", "good", len(good_list), "new", len(new_list), "normal", len(moves))
			return moves
